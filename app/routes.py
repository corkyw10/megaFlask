...
def edit_profile():
    ...
    if form.validate_on_submit():
        ...
        db.session.commit()
        flash("Your changes have been saved ranger!")
...
def follow(username):
    ...
    if form.validate_on_submit():
        ...
        if user is None:
            flash('Ranger {} not found.'.format(username))
            return redirect(url_for('index'))
        if user == current_user:
            flash('Rita Repulsa has tricked you! You can\'t follow yourself!')
...
def unfollow(username):
    ...
    if form.validate_on_submit():
        ...
        if user is None:
            ...
            return redirect(url_for('index'))
        if user == current_user:
            flash('Lord Zedd has tricked you! You can\'t follow yourself!')
            return redirect(url_for('user', username=username))
